...
const usage = 'Usage: node src/engine/cli.ts --fen "<six-field FEN>" [--moves e2e4,e7e5] [--engine /path/to/stockfish] [--movetime 800] [--nodes 100000] [--multipv 3] [--skill 0..20]';
...
if (args.includes('--help') || !value('--fen')) { console.error(usage); process.exitCode = args.includes('--help') ? 0 : 2; }
  ...
  try {
    const completed = await analyze(value('--fen')!, value('--moves')?.split(',').filter(Boolean) ?? [], {
      enginePath: value('--engine'), movetimeMs: numeric('--movetime'), nodes: numeric('--nodes'), multiPv: numeric('--multipv'), skillLevel: numeric('--skill')
    });
