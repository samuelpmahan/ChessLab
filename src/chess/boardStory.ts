import type {DebuggerSnapshot} from './debugger.ts';
import type {CandidateMove,AnalysisPiece,PieceRelations} from './analysis.ts';
export interface StoryArrow {from:string;to:string;kind:'move'|'preview'|'check'|'support';label:string}
export interface StoryMark {square:string;label:string;kind:'blocked'|'ghost'}
export interface BoardStory {title:string;arrows:StoryArrow[];marks:StoryMark[];notes:string[]}
export interface StoryFocus {candidate?:CandidateMove|null;piece?:AnalysisPiece|null;pieceRelation?:PieceRelations|null;nextMove?:{source:string;target:string;san?:string}|null}
/** Pure Story projection: reads retained constructions; never generates moves or scores. */
export function boardStory(snapshot:DebuggerSnapshot,focus:StoryFocus={},change?:{source:string;target:string;san?:string;capture?:unknown}|null):BoardStory {
 const story:BoardStory={title:'Position',arrows:[],marks:[],notes:[]};
 const add=(from:string,to:string,kind:StoryArrow['kind'],label:string)=>{
