...
import type {Color,PieceType} from './analysis.ts';
import type {Frame} from './stages/S0/clean/index.ts';

export interface StudyPiece {readonly square:string;readonly label:string;readonly id?:string}
export interface StudyFrame extends Frame {
 readonly title?:string;
 readonly note?:string;
 readonly change?:unknown;
 readonly pieces:StudyPiece[];
}
...
export function normalizeStudyLabel(value:string):string {
...
function colorOf(label:string):Color{return normalizeStudyLabel(label)[1]==='w'?'white':'black';}
function typeOf(label:string):PieceType{return TYPES[normalizeStudyLabel(label)[0]];}
function assertSquare(square:string){if(!/^[a-h][1-8]$/.test(square))throw Error('Square must be a1 through h8');}
function adjacent(a:string,b:string){return Math.max(Math.abs(a.charCodeAt(0)-b.charCodeAt(0)),Math.abs(Number(a[1])-Number(b[1])))<=1;}

/** Stable FEN metadata generated directly from the editable piece placement. */
export function studyFen(pieces:readonly StudyPiece[],sideToMove:Color):string {
...
export function prepareStudyFrame(frame:StudyFrame):StudyFrame {
...
export function studyPieceMatches(piece:StudyPiece,id:string):boolean {
 const wanted=String(id??'').trim();
 if(piece.id&&wanted.toUpperCase()===piece.id.toUpperCase())return true;
 try{
  const materialized=piece as StudyPiece&{type?:string;color?:string};
  const label=materialized.label??(materialized.type&&materialized.color
   ? `${({King:'K',Queen:'Q',Rook:'R',Bishop:'B',Knight:'N',Pawn:'P'} as Record<string,string>)[materialized.type]??materialized.type[0]}${materialized.color==='white'?'w':materialized.color==='black'?'b':''}`
   : '');
  return normalizeStudyLabel(wanted)===normalizeStudyLabel(label);
 }catch{return false;}
}

export function moveStudyPiece(frame:StudyFrame,id:string,square:string):StudyFrame {
 assertSquare(square);
...
export function turnStudyFrame(frame:StudyFrame,sideToMove:Color):StudyFrame {
