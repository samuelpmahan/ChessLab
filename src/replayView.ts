...
export function replayView(replay:ReturnType<typeof gameReplay>):string {
 const f=replay.current(),width=58,lines:string[]=[];
 const line=(text='')=>lines.push('│ '+text.padEnd(width)+' │');
 const rule=()=>lines.push('├'+'─'.repeat(width+2)+'┤');
 const wrap=(text:string)=>{let row='';for(const word of text.split(/\s+/)){if(row.length+word.length+1>width){line(row);row='';}row+=(row?' ':'')+word;}if(row)line(row);};
