...
export function debuggerView(run            , side     ='white')        {
 ...
 lines.push('Pieces: '+(selected.pieces ?? observed.pieces?.[side] ?? []).map((p    )=>`${p.id ?? p.label ?? p.type}=${p.square}`).join('  '));
 const candidates=[...(selected.candidates ?? [])].filter((c    )=>c.score).sort((a    ,b    )=>(b.score?.total??-Infinity)-(a.score?.total??-Infinity)).slice(0,5);
 ...
 lines.push('Check witnesses: '+(witnesses.length ? witnesses.map((w    )=>`${w.source} attacks ${w.target}`).join('; ') : 'none'));
 return lines.join('\n');
}
