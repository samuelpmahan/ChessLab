...
for(const path of ['state','views','lab/debugMaterializer','lab/board','lab/contract','lab/sha256']){let source=stripTypeScriptTypes(readFileSync('src/'+path+'.ts','utf8'));source=source.replace(/(from\s+['"][^'"]+)\.ts(['"])/g,'$1.js$2');writeFileSync('dist/src/'+path+'.js',source);}
